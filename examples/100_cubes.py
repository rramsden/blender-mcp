...
for i in range(grid_size):
    for j in range(grid_size):
        ...
        # Get the newly created cube using a more reliable method
        # Access the last object in the collection since it should be our new cube
        if len(bpy.data.objects) > 0:
            cube = bpy.data.objects[-1]  # Get the last added object
...
print(f"Created {grid_size * grid_size} cubes in a grid pattern")
