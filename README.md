...
## Files
- **blender_server.py** – Run inside Blender's Python environment. It starts a background thread that listens for commands on port `50007`. When it receives the command `render_cube` it creates a cube in the scene and renders an image (`cube_render.png`) next to your `.blend` file.
...
1. **Start Blender on Windows**
   ...
   - In the text editor, open `blender_server.py` (you can copy it from this repo into the same folder).
...
- Ensure that the port `50007` is not blocked by a firewall on Windows.
